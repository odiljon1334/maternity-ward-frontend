...
# Marketing (FAZA 4) — GTM/Meta Pixel, berilmasa TrackingScripts hech narsa ulamaydi
ARG NEXT_PUBLIC_GTM_ID
ENV NEXT_PUBLIC_GTM_ID=$NEXT_PUBLIC_GTM_ID
ARG NEXT_PUBLIC_META_PIXEL_ID
ENV NEXT_PUBLIC_META_PIXEL_ID=$NEXT_PUBLIC_META_PIXEL_ID
# Sentry (Faza 3) — berilmasa Sentry umuman ishga tushmaydi (xavfsiz standart)
...
# SENTRY_AUTH_TOKEN ENV sifatida saqlanmaydi (image konfiguratsiyasida
# qolardi). Afzal yo'l — BuildKit secret:
#   docker build --secret id=sentry_auth_token,env=SENTRY_AUTH_TOKEN ...
# Orqaga moslik: build-arg ham qabul qilinadi, lekin faqat shu RUN ichida.
