      ...
      <div className="relative card w-full max-w-md">
        ...
        >
          ...
          <div>
            ...
            <input
              {...register("payerName", { required: "Ism shart" })}
              className="input-field"
...
function EditPaymentModal({
  ...
  const mutation = useMutation({
    mutationFn: (d: { amount: number; note: string }) =>
      paymentsApi.update(payment.id, d),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["payments-overview"] });
      qc.invalidateQueries({ queryKey: ["payments-list"] });
      ...
      <div className="relative card w-full max-w-sm">
        ...
        <form
          onSubmit={handleSubmit((d) =>
            mutation.mutate({ amount: Number(d.amount), note: d.note }),
          )}
          className="p-6 space-y-4"
        >
          <div>
    ...
    <div className="card overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
        <div className="flex items-center gap-2">
          ...
          {totalDebtSum > 0 && (
            <span className="badge-gray text-red-400">
              {formatAmount(totalDebtSum)} qarz
            </span>
          )}
        </div>
        <select
      ...
      {isLoading ? (
        <div className="p-5 space-y-2">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="h-12 rounded-xl bg-[var(--bg-hover)] animate-pulse"
            />
          ))}
        </div>
      ) : withDebt.length === 0 ? (
            ...
            <thead>
              <tr className="border-b border-[var(--border)]">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-xs text-[var(--text-muted)] font-medium"
                  >
                    {h}
                  </th>
                ))}
              ...
              {withDebt.map((d) => (
                <React.Fragment key={d.id}>
                  <tr className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-[var(--text-muted)]">
                      {formatAmount(d.expectedAmountPerMonth)}
                    </td>
                    ...
                    <td className="px-4 py-3 font-semibold text-red-400">
                      {formatAmount(d.totalDebt)}
                    </td>
                    <td className="px-4 py-3 text-xs text-[var(--text-muted)]">
                      {d.lastPaymentAt
                        ? new Date(d.lastPaymentAt).toLocaleDateString("uz-UZ")
                        : "Hech qachon"}
                    </td>
                    <td className="px-4 py-3">
                      <button
                      ...
                      <td colSpan={7} className="px-4 py-3">
                        <div className="flex flex-wrap gap-3">
                          {d.monthly.map((m) => (
                            <div
                              key={m.period}
                              className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border)]"
                            >
                              ...
                              <span className="text-[var(--text-muted)]">
                                — {formatAmount(m.paidAmount)}/
                                {formatAmount(m.expectedAmount)}
                              </span>
                            </div>
                          ))}
                        </div>
...
// /dashboard/payments va /panel/payments bir xil production-sinalgan oqimdan
// foydalanadi. Shu bilan to'lov mantiqi ikki sahifada ko'chirib yurilmaydi.
    ...
    <div>
      {!embedded && (
        <Topbar
          ...
          subtitle="Muassasalar obuna to'lovlari"
        />
      )}
      ...
      <div
        className={
          embedded
            ...
            : "p-4 space-y-4 lg:p-6 lg:space-y-6"
        }
      >
        {/* Summary Cards */}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-[var(--text-primary)]">
              Muassasalar holati —{" "}
              {overview[0] ? periodLabel(overview[0].period) : ""}
            </h2>
            <button
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="card p-5 h-40 animate-pulse bg-[var(--bg-hover)]"
                />
              ))}
            </div>
          ) : (
                    ...
                    {h.status !== "PAID" && (
                      <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-2">
                        ...
                        <p className="font-bold text-red-400">
                          {formatAmount(h.remainingAmount)}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <button
                    onClick={() =>
                      setAddModal({
                        open: true,
                        hospitalId: h.id,
                        amount: h.remainingAmount || h.expectedAmount,
                      })
                    }
      ...
      <AddPaymentModal
        key={`${addModal.open}-${addModal.hospitalId || "all"}-${addModal.amount || 0}`}
        open={addModal.open}
